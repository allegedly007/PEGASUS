...
TWR_o= [init1.T_W, init2.T_W]'; 
unc_pos = ones(numel(TWR_o),3) .* [1.01 1.02 1.03] ; unc_pos = TWR_o .*unc_pos ; 
unc_neg = ones(numel(TWR_o),3) .* [0.97 0.98 0.99]  ; unc_neg = TWR_o .*unc_neg ; 

m_c_i = [unc_neg, TWR_o, unc_pos]; %TWR confidence inteval
...
for ii = 1:n
    [init1.T_W, init2.T_W] = deal(m_c_i(1,ii), m_c_i(2,ii));
...
subtitle('Thrust-to-Weight Ratio Uncertainty')
legend(["+/-0%",  "-1%", "-2%", "-3%" ,"+1%", "+2%", "+3%"], Location="northwest")
axis equal
grid on

figure
f = plot(Ox2(:,4) , Oy2(:,4), LineStyle="-", Color= 'k', LineWidth=2)
hold on 
